...
co_occurance_matrix = pd.DataFrame(0, index=candidates, columns=candidates)  # type: ignore

def get_classwise_documents_from_local()-> dict:
    return {}

classwise_votes = get_classwise_documents_from_local()
for _class in classwise_votes:
    for sessions in _class["votes"]:
